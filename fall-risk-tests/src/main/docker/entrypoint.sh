...
doListTests() {
  local TESTS_JAR=$(find -maxdepth 1 -name "fall-risk-tests-*-tests.jar")
  jar -tf $TESTS_JAR \
...
doListCategories() {
  local MAIN_JAR=$(find -maxdepth 1 -name "fall-risk-tests-*.jar" -a -not -name "fall-risk-tests-*-tests.jar")
  jar -tf $MAIN_JAR \
...
validateProvidedVariables() {
  ...
  for param in "K8S_LOAD_BALANCER" "FALL_RISK_API_PATH" "FALL_RISK_SMOKE_TEST_CATEGORY" "FALL_RISK_REGRESSION_TEST_CATEGORY"; do
...
doSmokeTest() {
  ...
  INCLUDE_CATEGORY="$FALL_RISK_SMOKE_TEST_CATEGORY"
  doTest
}
...
doRegressionTest() {
  ...
  INCLUDE_CATEGORY="$FALL_RISK_REGRESSION_TEST_CATEGORY"
  doTest
}
